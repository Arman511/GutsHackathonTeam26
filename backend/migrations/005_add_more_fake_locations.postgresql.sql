...
SELECT
    ...
    '' as address,
    '' as description,
    (floor(random()*5) + 1) as google_rating,
    CASE
        WHEN random() < 0.60 THEN '£'
        WHEN random() < 0.85 THEN '££'
        ELSE '£££' END as price_range,
    false as outdoor,
    false as group_activity,
    false as vegetarian,
    false as drinks,
    false as food,
    false as pet_friendly,
    false as accessible,
    false as formal_attire,
    false as reservation_needed,
    ...
    END as image_url
FROM generate_series(101,1100) AS g;
