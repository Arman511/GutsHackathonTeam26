...
from backend.models import CreateLocationRequest, LocationInfo, LocationRankingRequest, LocationSearchRequest, ReviewData, UserRankings
...
async def add_location(user: user_dependency, db: db_dependency, location_data: CreateLocationRequest):
    if user is None:
        raise HTTPException(status_code=401, detail="Authentication Failed")
...
async def search_locations(query: LocationSearchRequest, user: user_dependency, db: db_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail="Authentication Failed")
...
def rank_location(location_rank: LocationRankingRequest, user: user_dependency, db: db_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail="Authentication Failed")
    user_id = int(user["id"])
...
@location_router.get("/get_locations_for_event/{event_id}", status_code=status.HTTP_200_OK)
def get_location_for_event(event_id: int, user: user_dependency, db: db_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail="Authentication Failed")
    ...
    if not locations:
        raise HTTPException(status_code=404, detail="No locations found matching event criteria")
    return {"locations": locations}

@location_router.post("/add_google_reviews", status_code=status.HTTP_200_OK)
def add_google_reviews_to_location(location_id:int, user_review:str, user_rating:int, db: db_dependency):
    entry = ReviewData(
        location_id=location_id,
