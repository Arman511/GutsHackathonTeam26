  ...
  useEffect(() => {
    const loadLocations = async () => {
      try {
        ...
        const personalityTags = JSON.parse(
          localStorage.getItem('personality_tags') || '[]'
        );
        ...
        let formattedLocations = locs.map((loc) => {
          ...
          if (personalityTags.length > 0) {
            personalityTags.forEach((tag) => {
              ...
              if (
                ['brave', 'adventurous', 'spontaneous', 'chaotic'].includes(
                  lowerTag
                )
              ) {
          ...
          return {
            ...
            secondaryImage: '📍',
            review: loc.google_rating
              ? `Google Rating: ${loc.google_rating}/5.0 ⭐`
              : 'No reviews yet',
        ...
        if (personalityTags.length > 0) {
          ...
          console.log(
            'Locations sorted by personality match!',
            personalityTags
          );
        }
