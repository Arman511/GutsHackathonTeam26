  ...
  const loadResults = async (eventId) => {
    ...
    try {
      ...
      if (response.ranked_locations) {
        setEventResults({
          locations: response.ranked_locations.map(
            ([locationId, avgScore]) => ({
            ...
            <Stack direction='row' spacing={2} flexWrap='wrap'>
              {myEvents.map((evt) => (
                <Button
                  ...
                  variant={
                    selectedEventId === evt.id ? 'contained' : 'outlined'
                  }
                  size='large'
                >
                  {evt.event_name}
          ...
          {resultsLoading ? (
            ...
            <>
                          ...
                          >
                            ...
                            {location.average_ranking && (
                              <Typography variant='h5' color='warning.main'>
                                {'★'.repeat(
                                  Math.round(location.average_ranking)
                                )}
                                {'☆'.repeat(
                                  5 - Math.round(location.average_ranking)
                                )}
                              </Typography>
                            )}
                            <Typography variant='body1' color='text.secondary'>
                              ({location.total_rankings || 0} rating
                              {location.total_rankings !== 1 ? 's' : ''})
                            </Typography>
                          </Stack>
                        </Box>
                      </Box>
              ...
              {locations[0]?.average_ranking && (
                <Card
                  sx={{ mt: 4, bgcolor: 'primary.main', color: 'white', p: 3 }}
                >
                  <Typography variant='h5' gutterBottom fontWeight='bold'>
                    Recommendation
                  </Typography>
                  <Typography variant='h6'>
                    Based on ratings,{' '}
                    <strong>{locations[0].location_name}</strong> is the clear
                    favorite!
                  </Typography>
                </Card>
              )}
